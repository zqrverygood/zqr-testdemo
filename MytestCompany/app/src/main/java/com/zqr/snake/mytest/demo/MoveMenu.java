...
public class MoveMenu extends FrameLayout implements OnTouchListener,
  ...
  public void initView() {
    ...
    mMenuViewParams = new LayoutParams(
    ...
    this.mParentWidth = dm.widthPixels;
    this.mParentHeight = dm.heightPixels;
  ...
  public void inflateMenuLayout(int resource, ViewGroup root,
    ...
    mMenuLayout = LayoutInflater.from(mContext).inflate(resource, root,
        attachToRoot);
    mMenuLayoutParams = new LayoutParams(
        ViewGroup.LayoutParams.WRAP_CONTENT,
        ViewGroup.LayoutParams.WRAP_CONTENT);
  ...
  public void showInView(ViewGroup viewGroup) {
    ...
    if (mMenuViewParams != null) {
      // 修改起始位置,靠右居中
      mMenuViewParams.setMargins(mParentWidth-mMenuViewParams.width,(mParentHeight-mMenuViewParams.height)/2,0,0);
  ...
  public boolean onTouch(final View v, MotionEvent event) {
    ...
    int dt = v.getTop();
    int llength = dl;
    int rlength = this.mParentWidth - dr;
    int tlength = dt;
    int blength = this.mParentHeight - db;
  ...
  private void moveViewWithFinger(View v, MotionEvent event) {
    ...
    if (r > this.mParentWidth - mEdgeInRight) {
      r = this.mParentWidth - mEdgeInRight;
      l = r - v.getWidth();
    }
    if (t < mEdgeInTop) {
      t = mEdgeInLeft;
    ...
    if (b > this.mParentHeight - mEdgeInBottom) {
      b = this.mParentHeight - mEdgeInBottom;
      t = b - v.getHeight();
    }
  ...
  private void menuLayoutOnPosition(View menuLayout) {
    switch (mMenuLayoutPosition) {
      case MENULAYOUT_POSITION_TOP:
        menuLayout.layout((mParentWidth - menuLayout.getWidth()) / 2
                + mMenuLayoutPositionOffsetX, mEdgeInTop
                + mMenuLayoutPositionOffsetY,
            (mParentWidth + menuLayout.getWidth()) / 2
                + mMenuLayoutPositionOffsetX, mEdgeInTop
      ...
      case MENULAYOUT_POSITION_CENTER:
        menuLayout.layout((mParentWidth - menuLayout.getWidth()) / 2
            ...
            (mParentHeight - menuLayout.getHeight()) / 2
                + mMenuLayoutPositionOffsetY,
            (mParentWidth + menuLayout.getWidth()) / 2
                + mMenuLayoutPositionOffsetX,
            (mParentHeight + menuLayout.getHeight()) / 2
      ...
      case MENULAYOUT_POSITION_BOTTOM:
        menuLayout.layout((mParentWidth - menuLayout.getWidth()) / 2
                + mMenuLayoutPositionOffsetX, mParentHeight - mEdgeInBottom
                - menuLayout.getHeight() + mMenuLayoutPositionOffsetY,
            (mParentWidth + menuLayout.getWidth()) / 2
                + mMenuLayoutPositionOffsetX, mParentHeight
  ...
  public void setIconSize(int width,int height){
    ...
    mMenuViewParams = new LayoutParams(width,height);
